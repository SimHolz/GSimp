...
#' 
#' @description Initialize missing values using various methods
#' @param miss_data Data frame or matrix with missing values
#' @param method Initialization method: 'lsym' (left symmetric), 'qrilc' (quantile regression), or 'rsym' (right symmetric)
#' @return Data frame with initialized missing values
#' @details 
#' - lsym: draws samples from right tail and transforms to left tail
#' - qrilc: uses quantile regression imputation
#' - rsym: draws samples from left tail and transforms to right tail
...
#' Single Variable Gibbs Sampler Imputation
#' 
#' @description Performs Gibbs sampling imputation for a single variable
#' @param x Matrix of predictor variables
#' @param y Response variable with missing values
#' @param y_miss Original response with missing values
#' @param y_real True values (optional, for testing)
#' @param imp_model Imputation model function name
#' @param lo Lower bound for truncated normal
#' @param hi Upper bound for truncated normal
#' @param iters_each Number of iterations
#' @param gibbs Indices for Gibbs sampling monitoring
#' @return List containing:
#'   \item{y_imp}{Imputed values}
#'   \item{gibbs_res}{Gibbs sampling results array}
...
#' Multiple Variable Gibbs Sampler Imputation
#' 
#' @description Performs Gibbs sampling imputation for multiple variables
#' @param data_miss Data frame with missing values
#' @param iters_each Number of iterations per variable (single number or vector)
#' @param iters_all Number of complete iterations
#' @param initial Initialization method ('qrilc'/'lsym') or initialized data matrix
#' @param lo Lower bound(s) for truncated normal
#' @param hi Upper bound(s) for truncated normal
#' @param n_cores Number of cores for parallel processing
#' @param imp_model Imputation model function name
#' @param gibbs Data frame specifying monitoring positions (row, col)
#' @return List containing:
#'   \item{data_imp}{Imputed dataset}
#'   \item{gibbs_res}{Gibbs sampling results array}
#' @details 
#' The function implements a Gibbs sampling approach for multiple imputation:
#' 1. Initializes missing values
#' 2. Iteratively imputes each variable using other variables as predictors
#' 3. Supports parallel processing for faster computation
#' 4. Allows monitoring of specific positions during imputation
...
#' GSimp Imputation
#' 
#' @description Alias for multi_impute function
#' @inheritParams multi_impute
#' @return Same as multi_impute
